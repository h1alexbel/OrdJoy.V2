...
public class MixRepositoryImpl extends AbstractGenericCRUDRepository<Mix, Long>
    ...
    public Optional<Mix> findByTitle(String title) {
        ...
        return session.createQuery("select m from Mix m where m.title = :mixTitle",
                        Mix.class)
    ...
    @Override
    public List<Track> findTracksByMixTitle(String mixTitle, int limit, int offset) {
        Session session = sessionFactory.getCurrentSession();
        List<Track> tracks = session
                .createQuery("select t from Mix m join m.tracks t" +
                             " where m.title = :mixTitle order by t.id desc",
                        Track.class)
    ...
    @Override
    public List<MixReview> findMixReviewsByMixTitle(String mixName, int limit, int offset) {
        Session session = sessionFactory.getCurrentSession();
        ...
        List<MixReview> mixReviews = session.createQuery(criteria)
                .setMaxResults(limit)
                .setFirstResult(offset)
                .getResultList();
    ...
    @Override
    public Long getMixReviewWithMixTitlePredicateRecords(String mixTitle) {
        Session session = sessionFactory.getCurrentSession();
        return session.createQuery("select count(mr) from MixReview mr" +
                                   " join mr.mix m where m.title =:mixTitle", Long.class)
                .setParameter(MIX_TITLE_PARAM, mixTitle)
                .getSingleResult();
    }

    @Override
    public Long getTrackWithMixTitlePredicateRecords(String mixTitle) {
        Session session = sessionFactory.getCurrentSession();
        return session.createQuery("select count(t) from Track t" +
                                   " join t.mixes m where m.title=:mixTitle", Long.class)
                .setParameter(MIX_TITLE_PARAM, mixTitle)
                .getSingleResult();
    }
}